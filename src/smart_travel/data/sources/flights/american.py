"""American Airlines award flight browser source.

Uses Playwright to scrape award/miles flight results from aa.com.
...
_CITY_TO_IATA: dict[str, str] = {
    ...
    "phoenix": "PHX", "charlotte": "CLT", "philadelphia": "PHL",
}
...
class AmericanAwardSource(BaseFlightSource):
    """Scrapes award flight results from aa.com via Playwright."""
    ...
    async def search_flights(
        ...
        """Scrape aa.com for award flight results."""
        try:
            from playwright.async_api import async_playwright
        except ImportError:
            logger.warning("Playwright not installed — skipping American award search")
            return []
        ...
        try:
            async with async_playwright() as pw:
                ...
                try:
                    await page.wait_for_selector(
                        ".flight-results-card, [data-testid='flightCard'], "
                        ".aa-flight-result",
                        timeout=15000,
                    )
                except Exception:
                    logger.debug("American: no award flight results rendered")
                ...
                raw = await page.evaluate("""
                    () => {
                        ...
                        for (const card of cards) {
                            try {
                                const miles = (
                                    card.querySelector('.award-price, [data-testid="awardPrice"], .miles-cost')
                                    ?.textContent?.trim() || ''
                                );
                                const airline = (
                                    card.querySelector('.airline-name, [data-testid="airline"], .carrier-name')
                                    ?.textContent?.trim() || 'American Airlines'
                                );
                                const flightNum = (
                                ...
                                const times = (
                                    card.querySelector('.depart-time, [data-testid="departTime"]')
                                    ?.textContent?.trim() || ''
                                );
                                const duration = (
            ...
            logger.warning("American award scraping failed", exc_info=True)
    ...
    def _build_url(
        ...
        """Build an aa.com award search URL."""
        orig = _resolve_iata(origin)
        dest = _resolve_iata(destination)
        cabin_code = _CABIN_MAP.get(cabin_class, "coach")

        return (
            f"https://www.aa.com/booking/find-flights"
            f"?bookingType=award&locale=en_US&pax=1"
            f"&searchType=OneWay&origin={orig}&destination={dest}"
            f"&departDate={departure_date}&cabinType={cabin_code}"
        )
    ...
    def _parse_results(
        ...
        for item in raw:
            ...
            results.append({
                "source": "american_award",
                "flight_number": item.get("flightNum", ""),
                "airline": item.get("airline", "American Airlines"),
                "origin": origin,
