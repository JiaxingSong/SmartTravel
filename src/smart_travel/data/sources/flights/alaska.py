"""Alaska Airlines award flight browser source.

Uses Playwright to scrape award/miles flight results from alaskaair.com.
...
_CITY_TO_IATA: dict[str, str] = {
    ...
    "beijing": "PEK", "portland": "PDX", "anchorage": "ANC",
}
...
class AlaskaAwardSource(BaseFlightSource):
    """Scrapes award flight results from alaskaair.com via Playwright."""
    ...
    async def search_flights(
        ...
        """Scrape alaskaair.com for award flight results."""
        try:
            from playwright.async_api import async_playwright
        except ImportError:
            logger.warning("Playwright not installed — skipping Alaska award search")
            return []
        ...
        try:
            async with async_playwright() as pw:
                ...
                try:
                    await page.wait_for_selector(
                        ".flight-option-card, [data-testid='flight-option'], "
                        ".flight-result-row",
                        timeout=15000,
                    )
                except Exception:
                    logger.debug("Alaska: no award flight results rendered")
                ...
                raw = await page.evaluate("""
                    () => {
                        ...
                        for (const card of cards) {
                            try {
                                const miles = (
                                    card.querySelector('.miles-price, [data-testid="milesPrice"], .award-miles')
                                    ?.textContent?.trim() || ''
                                );
                                const airline = (
                                    card.querySelector('.airline-name, [data-testid="airline"], .carrier')
                                    ?.textContent?.trim() || 'Alaska Airlines'
                                );
                                const flightNum = (
            ...
            logger.warning("Alaska award scraping failed", exc_info=True)
    ...
    def _build_url(
        ...
        """Build an alaskaair.com award search URL."""
        ...
        return (
            ...
            f"&adults=1&awardBooking=true"
            f"&cabinClass={cabin_code}&oneWay=true"
        )
    ...
    def _parse_results(
        ...
        for item in raw:
            ...
            results.append({
                ...
                "airline": item.get("airline", "Alaska Airlines"),
                "origin": origin,
