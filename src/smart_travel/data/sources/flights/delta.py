"""Delta Air Lines award flight browser source.

Uses Playwright to scrape award/miles flight results from Delta.com.
...
_CITY_TO_IATA: dict[str, str] = {
    "new york": "JFK", "tokyo": "NRT", "london": "LHR",
    ...
    "beijing": "PEK", "houston": "IAH", "washington": "DCA",
}

_CABIN_MAP: dict[str, str] = {
    "economy": "MAIN",
    "premium_economy": "PREMIUM_SELECT",
    "business": "DELTA_ONE",
    "first": "FIRST",
}
...
class DeltaAwardSource(BaseFlightSource):
    """Scrapes award flight results from Delta.com via Playwright."""
    ...
    async def search_flights(
        ...
        """Scrape Delta.com for award flight results."""
        try:
            from playwright.async_api import async_playwright
        except ImportError:
            logger.warning("Playwright not installed — skipping Delta award search")
            return []
        ...
        try:
            async with async_playwright() as pw:
                ...
                try:
                    await page.wait_for_selector(
                        ".result-card, [data-testid='resultCardContainer'], "
                        ".flight-result",
                        timeout=15000,
                    )
                except Exception:
                    logger.debug("Delta: no award flight results rendered")
                ...
                raw = await page.evaluate("""
                    () => {
                        const selectors = [
                            '.result-card',
                            '[data-testid="resultCardContainer"]',
                            '.flight-result'
                        ];
                        let cards = [];
                        ...
                        for (const card of cards) {
                            try {
                                const miles = (
                                    card.querySelector('.award-miles-amount, [data-testid="award-miles"], .miles-amount')
                                    ?.textContent?.trim() || ''
                                );
                                const airline = (
                                    card.querySelector('.airline-name, [data-testid="airline-name"], .carrier')
                                    ?.textContent?.trim() || 'Delta Air Lines'
                                );
                                const flightNum = (
                                    card.querySelector('.flight-number, [data-testid="flightNumber"]')
                                    ?.textContent?.trim() || ''
                                );
                                const times = (
                                    card.querySelector('.departure-time, [data-testid="departTime"]')
                                    ?.textContent?.trim() || ''
                                );
                                const duration = (
                                ...
                                const stops = (
                                    card.querySelector('.stops-text, [data-testid="stops"]')
                                    ?.textContent?.trim() || ''
                                );
                                if (miles) {
                                    results.push({
            ...
            logger.warning("Delta award scraping failed", exc_info=True)
    ...
    def _build_url(
        ...
        """Build a Delta.com award search URL."""
        orig = _resolve_iata(origin)
        dest = _resolve_iata(destination)
        cabin_code = _CABIN_MAP.get(cabin_class, "MAIN")

        return (
            f"https://www.delta.com/flight-search/book-a-flight"
            f"?tripType=ONE_WAY&paxCount=1&awardTravel=true"
            f"&originCity={orig}&destinationCity={dest}"
            f"&departureDate={departure_date}"
            f"&cabinClass={cabin_code}&deltaOnlySearch=false"
        )
    ...
    def _parse_results(
        ...
        for item in raw:
            ...
            results.append({
                ...
                "airline": item.get("airline", "Delta Air Lines"),
                "origin": origin,
